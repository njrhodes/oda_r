...
One row per endpoint x actual class. The per-leaf `class_counts_raw` vectors
...
Stabilized propensity-style weights at the endpoint x actual-class level,
following Yarnold and Linden (2017):

```
weight_{s,z} = n_s x Pr(Z=z) / n_{s,z}
```
...
**Scope:** `cta_propensity_weights()` returns endpoint x actual-class weights  - 
one row per endpoint x class combination. `cta_assign_endpoints()` maps
caller-supplied rows to endpoint IDs on demand. `cta_observation_weights()`
joins endpoint assignment to the endpoint x class weights and returns one row
...
exactly 1.0. For endpoint 1: `(146 x 0.9093 + 36 x 1.3680) / 182 = 1.0`.
...
the resulting `endpoint_id` and `actual_class` to the endpoint x class weights
...
Each observation receives the endpoint x actual-class propensity weight for
...
1. **`training_confusion`**  -  a C x C integer matrix of raw classification
