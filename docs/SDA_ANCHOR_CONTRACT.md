...
$outcome_name             # NA_character_  ← not stored (see gap §9.2)
...
- **outcome_name:** Not stored in `sda_fit`.  The outcome variable name is
  not part of the fitted object (gap — see §9.2).
...
| `stage_table` | data frame | One row per SDA step (see §5.2) |
...
| `removal_history` | data frame | When `remove_correct = TRUE` (see §5.4) |
...
stated in `LORT_SORT_GORT_TAXONOMY.md §Agent Handoff Contract`.
...
7. Add tests A1–A15 (§10) in `tests/testthat/test-sda-anchor.R`.
