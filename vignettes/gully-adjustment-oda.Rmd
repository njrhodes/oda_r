...
(LOO) jackknife validity analysis is included. For categorical attributes the
`direction` parameter does not apply; the directional hypothesis is evaluated
by comparing the fitted rule to the hypothesized partition (see below).
...
- **MC p-value:** The printed `p(MC)` is a nondirectional Fisher-randomization
  p-value (categorical attribute). Interpret by decision threshold (e.g., p <
  0.05).
...
**Directional analysis.** The directional hypothesis — that Ridges and Shifting
predict Community motivation and Relocation and Intensification predict
Individual motivation — is a nominal-partition hypothesis. ODA's categorical
engine finds the globally optimal partition; the rule above matches the
hypothesis exactly, confirming it. MPE Chapter 4 categorical/table DIRECTIONAL
testing (where a specific partition is constrained *a priori* and the p-value
is one-tailed) is Phase 6C (not yet implemented). The current nondirectional
MC p-value is conservative relative to the directional alternative.
