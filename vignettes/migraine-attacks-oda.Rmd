...
(no categorical flag), consistent with the MegaODA reference analysis.
...
```{r confusion}
# Confusion matrix: actual treatment (rows) × predicted treatment (cols)
conf_mat <- matrix(
...
- **MC p-value:** The printed `p(MC)` is a nondirectional Fisher-randomization
  p-value. The MegaODA reference value is p ≈ 0.086 (not significant at α =
...
MegaODA's reported value (p ≈ 0.086). This is a known discrepancy under
investigation (issue \#5: MC STOP calibration audit). Training ESS and
confusion matrix are unaffected.

**Directional p-value.** The original analysis did not specify a directional
hypothesis *a priori*. Directional ODA is not yet implemented (issue \#6).
