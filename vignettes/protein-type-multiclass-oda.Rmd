...
**Directional analysis.** The original analysis specified a *directional*
hypothesis (convergent validity: amino acid type codes equal biological type
codes). This is a multiclass categorical directional hypothesis (MPE Chapter 4
TABLE/DIRECTIONAL). MPE Chapter 2 binary ordered directional ODA is implemented
via `direction = c("greater", "less")` in `oda_fit()` and `oda_univariate_core()`.
Multiclass and categorical/table DIRECTIONAL is Phase 6C (not yet implemented);
the current nondirectional MC p-value is conservative relative to the
