...
Amino acid type is a four-category nominal variable. Because the directional
hypothesis is convergent validity  -  amino acid type codes equal biological type
codes  -  and there are exactly as many attribute categories as class values
(k = C = 4), `direction = "ascending"` is used. This constrains ODA to the
identity mapping (amino acid type i -> biological type i) without searching
other partitions. ODA evaluates the identity rule and conducts the MC
permutation test under the same constraint  -  a directional hypothesis test
consistent with MPE Chapter 4 categorical identity-map DIRECTIONAL (Phase 6C).
Leave-one-out (LOO) jackknife validity is requested via `loo = "on"`; LOO
confusion and ESS are reported. No LOO p-value is given because no canonical
Fisher-exact LOO p-value is defined for C > 2 multicategorical class problems.
...
The directional MC p-value and LOO results are shown in the `print` and
`summary` output above.

- **MC p-value:** The printed `p(MC)` is a directional Fisher-randomization
  p-value. Because `direction = "ascending"` constrains each permutation to
  evaluate only the identity mapping, the p-value is one-tailed  -  consistent
  with the *a priori* convergent-validity hypothesis. Interpret by decision
  threshold (e.g., p < 0.05).
- **LOO jackknife:** Leave-one-out ESS and Mean PAC are shown. Each fold
  holds out one observation, searches for the optimal categorical mapping on
  the remaining n-1 observations (non-directional, equal priors), and
  classifies the held-out case using that fold's rule. Because the identity
  mapping is the globally optimal partition for these data, every fold
  recovers the same rule, and LOO ESS equals training ESS exactly. LOO ESS
  near training ESS indicates the model is stable  -  no single observation
  drives the result.
- **LOO p-value:** No LOO Fisher-exact p-value is reported for multicategorical
  ...
  the C×C LOO confusion matrix in this context. For binary class ODA, a
...
**Directional analysis.** `direction = "ascending"` implements MPE Chapter 4
categorical identity-map DIRECTIONAL (Phase 6C): with k = C = 4 attribute
categories and class values, `"ascending"` auto-creates the identity mapping
(amino acid type i -> biological type i) and constrains both training and MC
permutations to this mapping. Because the identity rule is the globally optimal
categorical mapping for this dataset, training ESS is unchanged relative to a
nondirectional analysis; only the MC p-value changes (directional p is at most
half the nondirectional p, and typically much smaller).
